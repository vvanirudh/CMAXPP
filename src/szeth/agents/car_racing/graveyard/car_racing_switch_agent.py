...
class car_racing_switch_agent(car_racing_agent):
    ...
    def learn_online_in_real_world(self,
        ...
        while True:
            ...
            if self.args.agent == 'aggressive':
                if min_qvalue < info['best_node_f']:
                    # Execute Q learning action
                    idx = qvalues.index(min_qvalue)
                    mprim = mprims[idx]
                    qlearning_mprim = True
            elif self.args.agent == 'probabilistic':
                # toss = self.rng.rand()
                # if toss < 0.2:
                #     idx = qvalues.index(min_qvalue)
                #     mprim = mprims[idx]
                #     qlearning_mprim = True
                # Trying out an informed version of sampling
                # If cmax value < min_qvalue then we should execute cmax action
                if info['best_node_f'] < min_qvalue:
                    print('V_t less than min_Q => Executing CMAX action')
                    qlearning_mprim = False
                else:
                    print('V_t greater than min_Q => Sample Q-learning acc. to gap')
                    gap = info['best_node_f'] - min_qvalue
                    # If gap is small, then CMAX is good
                    # If gap is large, then Q-learning is good
            elif self.args.agent == 'alternating':
                if not last_qlearning:
                    idx = qvalues.index(min_qvalue)
                    mprim = mprims[idx]
                    qlearning_mprim = True
                    last_qlearning = True
                else:
                    last_qlearning = False
            else:
