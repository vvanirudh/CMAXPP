...
from pr2_sim_pybullet.ss_pybullet.motion.motion_planners.rrt_connect import birrt, direct_path
...
TABLE_URDF = 'models/table_collision/table.urdf'
TABLE_URDF_2 = 'models/table_collision/table_2.urdf'
...
def create_flying_body(group, collision_id=NULL_ID, visual_id=NULL_ID, mass=STATIC_MASS):
    ...
    assert len(indices) == len(visuals) == len(
        collisions) == len(types) == len(parents)
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    for i, link in enumerate(links):
        ...
        collision_shapes.append(clone_collision_shape(
            body, link, client) if collision else NULL_ID)
        visual_shapes.append(clone_visual_shape(
            body, link, client) if visual else NULL_ID)
    ...
    new_body = p.createMultiBody(baseMass=base_dynamics_info.mass,
                                 baseCollisionShapeIndex=clone_collision_shape(
                                     body, base_link, client) if collision else NULL_ID,
                                 baseVisualShapeIndex=clone_visual_shape(
                                     body, base_link, client) if visual else NULL_ID,
...
def plan_waypoints_joint_motion(body, joints, waypoints, start_conf=None, obstacles=[], attachments=[],
    ...
    for i, waypoint in enumerate([start_conf] + list(waypoints)):
        if collision_fn(waypoint):
            print(
                "Warning: waypoint configuration {}/{} is in collision".format(i, len(waypoints)))
            return None
    path = [start_conf]
...
def check_conf(conf, collision_fn):
...
def plan_joint_motion_2(body, joints, ee_link, end_conf, obstacles=[], attachments=[],
                        self_collisions=True, disabled_collisions=set(),
                        weights=None, resolutions=None, max_distance=MAX_DISTANCE, custom_limits={}, **kwargs):
    ...
    if not check_conf(end_conf, collision_fn):
        print("Warning: end configuration is in collision")
        return None
    if not check_conf(start_conf, collision_fn):
        print("Warning: start configuration is in collision")
        # Start conf in collision
        # Perturb joints so that it is out of collision
        # start_conf = sample_close_collision_free_conf(
        #     body, joints, ee_link, start_conf, collision_fn)
        modified_collision_fn = modify_collision_fn(collision_fn, start_conf)
        return birrt(start_conf, end_conf, distance_fn,
                     sample_fn, extend_fn, modified_collision_fn, **kwargs)
    return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)


def sample_close_collision_free_conf(body, joints, ee_link, conf, collision_fn):
    print('Perturbing start conf to get out of collision', flush=True)
    set_joint_positions(body, joints, conf)
    print('given conf', conf, flush=True)
    # get end effector position
    ee_position, ee_orientation = get_link_pose(body, ee_link)
    # Sample small deviations to joint positions
    while True:
        deviation = np.random.randn(len(joints)) * 0.001
        sampled_conf = np.array(conf) + deviation
        print('sampled conf', sampled_conf, flush=True)
        # Check if this violates joint limits
        if violates_limits(body, joints, sampled_conf):
            print('Violates limits', flush=True)
            continue
        # Check if this is in collision with anything
        if not check_conf(sampled_conf, collision_fn):
            print('In collision', flush=True)
            continue
        # Check if the ee position is not too far
        set_joint_positions(body, joints, sampled_conf)
        sampled_ee_position, sampled_ee_orientation = get_link_pose(
            body, ee_link)
        distance = get_distance(ee_position, sampled_ee_position)
        distance += get_distance(ee_orientation, sampled_ee_orientation)
        if distance > 1e-3:
            print('ee too far', flush=True)
            continue
        break
    return sampled_conf


def modify_collision_fn(collision_fn, start_conf):
    def modified_collision_fn(q):
        if get_distance(q, start_conf) < 0.005:
            return False
        return collision_fn(q)
    return modified_collision_fn
...
def control_joints_1(body, joints, positions):
    # TODO: the whole PR2 seems to jitter
    # kp = 1.0
    # kv = 0.3
    # forces = np.array([get_max_force(body, joint) for joint in joints])
...
def control_joints(body, joints, positions):
    ...
    kp = np.zeros_like(forces)
    kv = np.zeros_like(forces)
    forces[0] = forces[1] = forces[2] = 1e6  # Base x, y, z
    kp[0] = kp[1] = kp[2] = 0.1
    kv[0] = kv[1] = kv[2] = 1.0
    forces[3] = 1e6  # torso_lift_joint
    kp[3] = 0.1
    kv[3] = 1.0
    forces[4] = forces[5] = 1e6  # head joints
    kp[4] = kp[5] = 0.1
    kv[4] = kv[5] = 1.0
    # forces[6] = forces[17] = 1e6  # shoulder pan
    kp[6] = kp[17] = 2400  # shoulder pan
    kv[6] = kv[17] = 18
    # forces[7] = forces[18] = 1e6  # shoulder lift
    kp[7] = kp[18] = 1200  # shoulder lift
    kv[7] = kv[18] = 10
    kp[8] = kp[19] = 1000  # upper arm roll
    kv[8] = kv[19] = 6
    kp[9] = kp[20] = 700  # elbow flex
    kv[9] = kv[20] = 4
    kp[10] = kp[21] = 300  # forearm roll
    kv[10] = kv[21] = 6
    kp[11] = kp[22] = 300  # wrist flex
    kv[11] = kv[22] = 4
    kp[12] = kp[23] = 300  # wrist roll
    kv[12] = kv[23] = 4
    kp[13] = kp[14] = kp[15] = kp[16] = 1e4  # gripper joints
    kv[13] = kv[14] = kv[15] = kv[16] = 1e3  # gripper joints
    kp[24] = kp[25] = kp[26] = kp[27] = 1e4  # gripper joints
    kv[24] = kv[25] = kv[26] = kv[27] = 1e3  # gripper joints
    # return p.setJointMotorControlArray(body, joints, p.POSITION_CONTROL,
    #                                    targetPositions=positions,
    #                                    targetVelocities=[0.0] * len(joints),
    #                                    physicsClientId=CLIENT,
    ...
    for i in range(len(joints)):
        p.setJointMotorControl2(body, joints[i], p.POSITION_CONTROL,
    ...
    for target_pose in waypoint_poses:
        for iteration in range(max_iterations):
            ...
            if is_pose_close(get_link_pose(sub_robot, selected_target_link), target_pose, **kwargs):
                ...
                kinematic_conf = np.round(kinematic_conf, decimals=3)
                if not all_between(lower_limits, kinematic_conf, upper_limits):
                    # movable_joints = get_movable_joints(robot)
