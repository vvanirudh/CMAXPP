...
from .utils import invert, multiply, get_name, set_pose, get_link_pose, is_placement, \
    ...
    set_base_values, euler_from_quat, INF, elapsed_time, get_moving_links, flatten_links, joint_controller
...
class Trajectory(Command):
    ...
    def apply(self, state, sample=1):
        handles = add_segments(
            self.to_points()) if self._draw and has_gui() else []
    ...
    def control(self, dt=0, **kwargs):
        ...
        for conf in self.path:
            ...
            for _ in joint_controller_hold(conf.body, conf.joints, conf.values):
                # for _ in joint_controller(conf.body, conf.joints, conf.values):
                print('Moving joints', conf.joints, 'to values', conf.values)
                step_simulation()
                time.sleep(dt)
...
class GripperCommand(Command):
    ...
    def control(self, **kwargs):
        ...
        for _ in joint_controller_hold(self.robot, joints, positions):
            # for _ in joint_controller(self.robot, joints, positions):
            yield
...
class Attach(Command):
    ...
    def __init__(self, robot, arm, grasp, body):
        ...
        self.grasp = grasp
        self.body = body
        self.link = link_from_name(
            self.robot, PR2_TOOL_FRAMES.get(self.arm, self.arm))
    ...
    def control(self, dt=0, **kwargs):
        if self.vacuum:
            ...
            values = [get_min_limit(self.robot, joint)
                      for joint in joints]  # Closed
            for _ in joint_controller_hold(self.robot, joints, values):
                # for _ in joint_controller(self.robot, joints, values):
                step_simulation()
                time.sleep(dt)
...
def get_grasp_gen(problem, collisions=False, randomize=True):
    ...
    def fn(body):
        ...
        filtered_grasps = []
        for grasp in grasps:
            grasp_width = compute_grasp_width(
                problem.robot, arm, body, grasp.value) if collisions else 0.0
...
def get_ir_sampler(problem, custom_limits={}, max_attempts=25, collisions=True, learned=True):
    ...
    def gen_fn(arm, obj, pose, grasp):
        pose.assign()
        approach_obstacles = {
            obst for obst in obstacles if not is_placement(obj, obst)}
        for _ in iterate_approach_path(robot, arm, gripper, pose, grasp, body=obj):
            if any(pairwise_collision(gripper, b) or pairwise_collision(obj, b) for b in approach_obstacles):
                return
        gripper_pose = multiply(pose.value, invert(
            grasp.value))  # w_f_g = w_f_o * (g_f_o)^-1
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(arm, obj, pose, grasp, base_conf):
        ...
        grasp_conf = pr2_inverse_kinematics(
            robot, arm, gripper_pose, custom_limits=custom_limits)  # , upper_limits=USE_CURRENT)
        ...
        cmd = Commands(State(attachments=attachments), savers=[
                       BodySaver(robot)], commands=[mt])
        return (cmd,)
    return fn
...
def get_ik_ir_gen(problem, max_attempts=25, learned=True, teleport=False, **kwargs):
    ...
    ir_sampler = get_ir_sampler(
        problem, learned=learned, max_attempts=max_attempts, **kwargs)
    ...
    def fn(bq1, bq2):
        ...
        if teleport:
            ...
            if raw_path is None:
                print('Failed motion plan!')
                return None
            path = [Pose(robot, pose_from_base_values(q, bq1.value))
                    for q in raw_path]
        bt = Trajectory(path)
    ...
    def gen(arm, button):
        ...
        while True:
            for _ in range(max_attempts):
                ...
                # w_f_g = w_f_o * (g_f_o)^-1
                gripper_pose = multiply(pose, invert(press_pose))
                # approach_pose = gripper_pose # w_f_g * g_f_o * o_f_a = w_f_a
                approach_pose = multiply(gripper_pose, invert(
                    multiply(press_pose, approach)))
