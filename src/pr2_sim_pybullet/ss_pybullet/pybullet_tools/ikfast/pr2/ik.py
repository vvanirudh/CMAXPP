...
# UPPER_JOINT = {
#     'left': 'l_shoulder_lift_joint',
#     'right': 'r_shoulder_lift_joint',
# }
# UPPER_JOINT = {
#     'left': 'l_forearm_roll_joint',  # Third arm joint
#     'right': 'r_forearm_roll_joint',
# }
...
def is_ik_compiled():
    try:
        from pr2_sim_pybullet.ss_pybullet.pybullet_tools.ikfast.pr2.ikLeft import leftIK
        from pr2_sim_pybullet.ss_pybullet.pybullet_tools.ikfast.pr2.ikRight import rightIK
        return True
    except ImportError:
        return False
...
def get_ik_generator(robot, arm, ik_pose, torso_limits=USE_ALL, upper_limits=USE_ALL, custom_limits={}):
    ...
    sampled_joints = [joint_from_name(robot, name)
                      for name in [TORSO_JOINT, UPPER_JOINT[arm]]]
    sampled_limits = [get_ik_limits(robot, joint, limits) for joint, limits in zip(
        sampled_joints, [torso_limits, upper_limits])]
    arm_joints = get_torso_arm_joints(robot, arm)

    min_limits, max_limits = get_custom_limits(
        robot, arm_joints, custom_limits)
    while True:
        sampled_values = [random.uniform(*limits) for limits in sampled_limits]
        confs = compute_inverse_kinematics(
            arm_ik[arm], base_from_ik, sampled_values)
        solutions = [q for q in confs if all_between(
            min_limits, q, max_limits)]
...
def pr2_inverse_kinematics(robot, arm, gripper_pose, obstacles=[], custom_limits={}, use_ikfast=False, **kwargs):
