...
# high_def_optical_frame | high_def_frame | wide_stereo_l_stereo_camera_frame
...
def get_gripper_conf(robot, arm):
    return get_joint_positions(robot, get_gripper_joints(robot, arm))


def set_gripper_conf(robot, arm, conf):
    set_joint_positions(robot, get_gripper_joints(robot, arm), conf)
...
def get_top_cylinder_grasps(body, tool_pose=TOOL_POSE, body_pose=unit_pose(),
    ...
    center, (diameter, height) = approximate_as_cylinder(
...
def get_side_cylinder_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
    ...
    while True:
        theta = random.uniform(0, 2*np.pi)
        translate_rotate = ([x_offset, 0, diameter / 2 -
                             grasp_length], quat_from_euler([theta, 0, 0]))
        for j in range(1 + under):
            swap_xz = Pose(euler=[0, -math.pi / 2 + j * math.pi, 0])
...
def get_cylinder_push(body, theta, under=False, body_quat=unit_quat(),
    ...
    translate_x = Pose(point=[-diameter / 2 - side_offset, 0, 0])
...
def get_x_presses(body, max_orientations=1, body_pose=unit_pose(), top_offset=PRESS_OFFSET):
    ...
    for j in range(max_orientations):
        ...
        press_poses += [multiply(TOOL_POSE, translate,
                                 swap_xz, translate_center, body_pose)]
    return press_poses


def get_top_presses(body, tool_pose=TOOL_POSE, body_pose=unit_pose(), top_offset=PRESS_OFFSET, **kwargs):
    center, (_, height) = approximate_as_cylinder(
        body, body_pose=body_pose, **kwargs)
    reflect_z = Pose(euler=[0, math.pi, 0])
...
def attach_viewcone(robot, head_name=HEAD_LINK_NAME, depth=MAX_VISUAL_DISTANCE,
    ...
    for v1, v2 in cone_wires_from_support(get_viewcone_base(
        ...
        if is_optical(head_name):
            ...
            rotation = Pose(euler=Euler(roll=-np.pi/2, yaw=-np.pi/2))
        p1 = tform_point(rotation, v1)
        p2 = tform_point(rotation, v2)
        lines.append(add_line(p1, p2, color=color, parent=robot,
                              parent_link=head_link, **kwargs))
    return lines
...
def inverse_visibility(pr2, point, head_name=HEAD_LINK_NAME,
    ...
    camera_axis = np.array([0, 0, 1]) if is_optical(
        head_name) else np.array([1, 0, 0])
...
def get_detections(pr2, p_false_neg=0, camera_link=HEAD_LINK_NAME,
    ...
    for body in get_bodies():
        if (pr2 == body) or (np.random.random() < p_false_neg):
            continue
        mesh, z = get_detection_cone(
            pr2, body, camera_link=camera_link, **kwargs)
        if mesh is None:
            continue
...
def close_until_collision(robot, gripper_joints, bodies=[], open_conf=None, closed_conf=None, num_steps=25, return_path=False, **kwargs):
    ...
    resolutions = np.abs(np.array(open_conf) -
                         np.array(closed_conf)) / num_steps
...
def create_gripper(robot, arm, visual=True):
    ...
    gripper = clone_body(robot, links=links, visual=False,
